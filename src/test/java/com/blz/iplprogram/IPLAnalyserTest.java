...
public class IPLAnalyserTest {
	...
	public static final String Wtcs_FILE_PATH = "E:\\Fellowship\\STS\\CensusAnalyserFile\\CorrectWkts.csv";
	...
	public void givenIPLInfo_ShouldReturnTopBowlingAverages() {
		String sortedCensusData = null;
		try {
			sortedCensusData = iplAnalyser.getTopBattingAverages(Wtcs_FILE_PATH);
	...
	public void givenIPLInfo_ShouldReturnTopBowlingStrikeRates() {
		String sortedCensusData = null;
		try {
			sortedCensusData = iplAnalyser.getTopBowlingStrikeRates(Wtcs_FILE_PATH);
			IPLMostWktsCSV[] censusCSV = new Gson().fromJson(sortedCensusData, IPLMostWktsCSV[].class);
	...
	public void givenIPLInfo_ShouldReturnBestBowlingEconomyCricketerr() throws IOException, CensusAnalyserException {
		Assert.assertEquals("Ben Cutting",
				new IPLAnalyser().getBestBowlingEconomyCricketer(Wtcs_FILE_PATH).get(0).player);
	}
	...
	public void givenIPLInfo_ShouldReturnBestBowlingStrikingRatesWith5wN4w()
		...
		Assert.assertEquals("Lasith Malinga",
				new IPLAnalyser().bestBowlingStrikingRatesWith5wN4w(Wtcs_FILE_PATH).get(0).player);
	}
	...
	public void givenIPLInfo_ShouldReturnGreatBowlingAveragesWithBestStrikingRates()
		...
		Assert.assertEquals("Krishnappa Gowtham",
				iplAnalyser.greatBowlingAveragesWithBestStrikingRates(Wtcs_FILE_PATH).get(0).player);
	}

	@Test
	public void givenIPLInfo_ShouldReturnMaximumWicketsWithBestBowlingAverages()
			throws IOException, CensusAnalyserException {
		Assert.assertEquals("Imran Tahir",
				iplAnalyser.maximumWicketsWithBestBowlingAverages(Wtcs_FILE_PATH).get(0).player);
	}

	@Test
	public void givenIPLInfo_ShouldReturnBestBattingAverageWithBestBowlingAverage()
			throws IOException, CensusAnalyserException {
		Assert.assertEquals("Andre Russell", iplAnalyser.BestBattingAverageWithBestBowlingAverage().get(0));
	}

	@Test
	public void givenIPLInfo_ShouldReturnallRoundersCricketer() throws IOException, CensusAnalyserException {
		Assert.assertEquals("Andre Russell", iplAnalyser.allRoundersCricketer().get(0));
	}

	@Test
	public void givenIPLInfo_ShouldReturnMaximum100sWithBestBattingAverage()
			throws IOException, CensusAnalyserException {
		Assert.assertEquals("David Warner",
				iplAnalyser.maximum100sWithBestBattingAverage(Runs_FILE_PATH).get(0).player);
	}

	@Test
	public void givenIPLInfo_ShouldReturnhitZero100sN50sButHadBestBattingAverage()
			throws IOException, CensusAnalyserException {
		Assert.assertEquals("Marcus Stoinis",
				iplAnalyser.hitZero100sN50sButHadBestBattingAverages(Runs_FILE_PATH).get(0).player);
	}
}
