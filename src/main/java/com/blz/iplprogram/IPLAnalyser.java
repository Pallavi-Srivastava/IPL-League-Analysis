...
public class IPLAnalyser {
	...
	public static Map<IPLMostRunsCSV, IPLMostWktsCSV> iplCsvList;
	...
	public List<IPLMostWktsCSV> maximumWicketsWithBestBowlingAverages(String csvFilePath)
			throws IOException, CensusAnalyserException {
		loadWktsCSVData(csvFilePath);
		...
		double playerWithBestBowlingAverages = playerWithMaxWkts.stream().map(IPLMostWktsCSV::getAvg)
				.max(Double::compare).get();
		return playerWithMaxWkts.stream().filter(player -> player.getAvg() == playerWithBestBowlingAverages)
				.collect(Collectors.toList());
	}
	...
	public List<String> BestBattingAverageWithBestBowlingAverage() throws IOException, CensusAnalyserException {
		...
		List<IPLMostRunsCSV> playerWithBestBattingAvg = iplRunsCSVList.stream()
		...
		for (IPLMostRunsCSV iplMostRunsCSV : playerWithBestBattingAvg) {
			for (IPLMostWktsCSV iplMostWktsCSV : playerWithBestBowlingAvg) {

				if (iplMostRunsCSV.player.equals(iplMostWktsCSV.player)) {
					avgList.add(iplMostRunsCSV.player);
				}
			}
		}
		return avgList;
	...
	public List<String> allRoundersCricketer() throws IOException, CensusAnalyserException {

		List<String> bestOneCricketerList = new ArrayList<>();
		List<IPLMostRunsCSV> playerWithBestBattingRuns = iplRunsCSVList.stream()
		...
		for (IPLMostRunsCSV iplMostRunsCSV : playerWithBestBattingRuns) {
			for (IPLMostWktsCSV iplMostWktsCSV : playerWithBestBowlingWkts) {

				if (iplMostRunsCSV.player.equals(iplMostWktsCSV.player)) {
					bestOneCricketerList.add(iplMostRunsCSV.player);
				}
			}
		}
		return bestOneCricketerList;
	}

	public List<IPLMostRunsCSV> maximum100sWithBestBattingAverage(String csvFilePath)
			throws IOException, CensusAnalyserException {
		loadRunsCSVData(csvFilePath);
		int century = iplRunsCSVList.stream().map(IPLMostRunsCSV::getCentury).max(Integer::compare).get();
		List<IPLMostRunsCSV> maxCentury = iplRunsCSVList.stream().filter(player -> player.getCentury() == century)
				.collect(Collectors.toList());
		double battingAverage = maxCentury.stream().map(IPLMostRunsCSV::getAvg).max(Double::compare).get();
		return maxCentury.stream().filter(player -> player.getAvg() == battingAverage).collect(Collectors.toList());
	}

	public List<IPLMostRunsCSV> hitZero100sN50sButHadBestBattingAverages(String csvFilePath)
			throws IOException, CensusAnalyserException {
		loadRunsCSVData(csvFilePath);
		List<IPLMostRunsCSV> playerWithout100sN50s = iplRunsCSVList.stream()
				.filter(player -> (player.getCentury() + player.getHalfCentury()) == 0).collect(Collectors.toList());
		double playerWithBestAvg = playerWithout100sN50s.stream().map(IPLMostRunsCSV::getAvg).max(Double::compare)
				.get();
		return playerWithout100sN50s.stream().filter(player -> player.getAvg() == playerWithBestAvg)
				.collect(Collectors.toList());
	}
}